...
import { TVShow, fetchPopularTVShows } from "@/lib/tmdb";
...
const Tvshows = () => {
  ...
  const loadPage = useCallback(async (nextPage: number, append = false) => {
    try {
      ...
      const data = await fetchPopularTVShows(nextPage);
      setPage(data.page);
      setTotalPages(data.total_pages);
      const filtered = data.results.filter((t) => !!t.poster_path);
      setShows((prev) => (append ? [...prev, ...filtered] : filtered));
      setError(null);
    } catch (e) {
  ...
  return (
    <View className="flex-1">
      <AppHeader subTitle="Tvshows" title="CineFlix" />
      <FlatList
        data={shows}
